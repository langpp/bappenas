...
def makeChart(host, username, password, db, port, table, judul, name, subjudul, rangtanggalheader):
    try:        
        ...
        for row in mobile_records:
            ...
            postgreSQL_select_Query = "SELECT date, price, market_id, commodity_id, quality_id FROM "+table+" WHERE commodity_id='"+str(row[1])+"' AND quality_id='"+str(row[2])+"' AND market_id='"+str(row[0])+"' ORDER BY market_id ASC"
            cursor.execute(postgreSQL_select_Query)
